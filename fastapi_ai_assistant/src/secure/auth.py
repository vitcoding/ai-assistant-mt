...
class UpgradeAPIKeyCookie(APIKeyCookie):
    ...
    def get_current_user(token) -> UUID:
        ...
        """
        try:
            payload = jwt.decode(token, config.auth.secret_key, algorithms=[config.auth.algoritm])
        except JWTError:
        ...
        if not user_id:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='The user ID was not found')
